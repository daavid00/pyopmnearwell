...
[${tperiod},${tperiod},1,${40000*(-1)**(i+1)}${",3.6e1,'0.01 0.1 1e-12']," if -1==(-1)**(i+1) else ",'0.01 0.1 1e-12'],"}
% endfor
% if time%tperiod != 0:
[${time - tperiod*mt.floor(time/tperiod)},${time - tperiod*mt.floor(time/tperiod)},1,${40000*(-1)**(mt.floor(time/tperiod)+1)}${",3.6e1,'0.01 1 1e-12']" if -1==(-1)**(mt.floor(time/tperiod)+1) else ",'0.01 1 1e-12']"}
% endif
