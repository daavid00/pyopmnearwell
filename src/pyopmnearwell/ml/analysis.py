...
def sensitivity_analysis(
    ...
    for i in range(num_inputs):
        ...
        for j, fixed_input in enumerate(fixed_inputs):
            ...
            fixed_input: np.ndarray = np.tile(fixed_input, (resolution_2, 1))
...
def plot_analysis(
    ...
    **kwargs,
    ...
    num_rows: int = outputs.shape[0] // kwargs.get("max_columns", 3)
    ...
    for i, ax in enumerate(axes.flatten()):
        ...
        for j, color in enumerate(plt.cm.Blues(np.linspace(0.3, 0.7, outputs.shape[1]))):  # type: ignore
            ax.plot(
                inputs[i, j, :, i],
                outputs[i, j],
                color=color,
